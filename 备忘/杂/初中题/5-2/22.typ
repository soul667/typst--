...
= 22 题
图12-1是文具店正在销售的某种文件夹，图12-2为该文件夹装人纸张前后的纵横面示意图，已知纸张与龙骨截线PH垂直，且PH垂直于底板N,PH=6mm,夹
纸板截线PB与扣板截线PA的夹角始终保持特135°
如图12-2（甲），未装入纸张时，点B落在MN上，此时∠BPH=30°
如图12-2（乙），装满纸张时，点A落在AN上，此时∠APH=45°

#figure(
  image("./img/1.png", width: 100%),
)
借助以上信息，解决下列问题：（计算结果保留根号）

== (3)直接写出未装入纸张时A,H 两点之间的距离.
...
$ A H = sqrt((6-3)^2+ (6+ 3 sqrt(3))^2)=6 sqrt(2+ sqrt(3)) $ // $ A H = M N = 6 + 2 + 3 =11 $
